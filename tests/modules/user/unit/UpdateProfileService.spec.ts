...
describe('Update profile', () => {
  ...
  it('should be able to reset a password', async () => {
    ...
    const updatedUser = await updateProfile.execute({ user_id: id, name, email, old_password: '123456', password: newPassword })
  ...
  it('should not be Update with User not exist', async () => {
    ...
    expect(updateProfile.execute
      ({ user_id: '1', name, email, old_password: password, password: newPassword })
    )
      .rejects
